...
export default function AvatarFrame({ children, size = 48, active = true, dur = '2.6s', className }) {
  ...
  const canto = (extra, delay) => ({
    ...
    pointerEvents: 'none',
    zIndex: 2,
    ...(active ? { animation: `snakeGlow ${dur} ease-in-out ${delay} infinite` } : { opacity: 0.4 }),
    ...extra,
  });

  // Delays em sequência horária: sup-esq → sup-dir → inf-dir → inf-esq.
  return (
    <div className={className} style={{ position: 'relative', display: 'inline-block', borderRadius: 6 }}>
      ...
      <div style={canto({ bottom: -2, right: -2, borderBottom: '2px solid #d4a017', borderRight: '2px solid #d4a017', borderRadius: '0 0 2px 0' }, '1s')} />
      <div style={canto({ bottom: -2, left: -2, borderBottom: '2px solid #d4a017', borderLeft: '2px solid #d4a017', borderRadius: '0 0 0 2px' }, '1.5s')} />
    </div>
  );
}
